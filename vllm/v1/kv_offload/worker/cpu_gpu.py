...
def compute_sub_block_ptrs(
    ...
    if block_size_factor == 1:
        ...
        if current_platform.is_xpu():
            output[:] = (
                base_ptr
                + block_ids[:num_sub_blocks].astype(block_ptr_dtype) * row_stride
            )
        else:
...
class SingleDirectionOffloadingHandler(OffloadingHandler):
    ...
    def transfer_async(self, job_id: int, transfer_spec: TransferSpec) -> bool:
        ...
        all_src = np.empty(num_copy_ops, dtype=self.block_ptr_dtype)
        all_dst = np.empty(num_copy_ops, dtype=self.block_ptr_dtype)
        all_sizes = np.empty(num_copy_ops, dtype=self.block_ptr_dtype)
